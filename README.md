...
当前版本是 **V0.2.7：代码审查修补版本**。

V0.2.7 不改变采集产品路线，重点修补深度 Code Review 发现的稳定性和安全问题：package 资料包脚本不再因缺 `os` 崩溃；agentCLI ASR 依赖探测缺失时不再报错；SQLite 新连接统一加锁等待；采集任务日志只返回尾部 200KB；retention 非法输入返回 422；每日资料包 inbox 会在 Web 查询前自动同步到 SQLite；后台采集任务增加 2 小时总超时；采集操作台无活跃任务时降为 30 秒轮询。

V0.2.5 在 V0.2.4 Web 后台基础上，开始把第一部分“情报收集站”的核心采集能力拆成两个可独立运行的子项目：

- `aetherflux_shellCLI`：脚本和 OpenCLI 主导采集，agent 做监工、筛选和诊断。
- `aetherflux_agentCLI`：agent 主导采集，OpenCLI 和脚本作为辅助工具；已接通小红书/抖音真实采集、最新/一天内筛选和每日资料包输出。

两个模式都要能产出同一种每日资料包，供后续“超级智脑”读取。资料包会在子项目本地保留一份，也可以复制到统一数据目录 `AETHERFLUX_DATA_ROOT/daily_bundles_inbox/{mode}/{date}/{run_id}/`（默认 `~/Documents/Agent/AetherFlux_Data/`）。

V0.2.4 已把 V0.1.0 的旧静态 Web 后台换成新的 **React/Vite 前端 + FastAPI 后端**。后台第一屏是“采集作战台”，优先服务小红书/抖音的标题池、初筛、ASR 深处理、任务日志、回收站和系统诊断。

V0.2.5 继续沿用 V0.2.3 的“最近 24 小时标题池 → Hermes 初筛 → 本地 ASR 深处理”采集路线，并把采集执行拆进两个独立子项目：
...
- agentCLI 会优先从抖音详情页的浏览器媒体地址下载视频，`yt-dlp` 是备用路径；ASR 后会继续判断 `information_value`，避免纯配乐或低信号视频进入高价值情报
- 原始视频、音频、转写和证据仍只保存在本地或未来 NAS
- 从 V0.2.3 起，每个正式版本都必须同步 GitHub、打 tag，并尽量创建 GitHub Release

视频号仍因没有稳定网页端内容入口暂时跳过。V0.2.5 会保留禁用占位，但不进入真实采集队列。
...
```bash
cd aetherflux_shellCLI
python3 -m aetherflux_shellcli.cli run --main-inbox "$AETHERFLUX_DATA_ROOT"/daily_bundles_inbox

cd ../aetherflux_agentCLI
python3 -m aetherflux_agentcli.cli run --main-inbox "$AETHERFLUX_DATA_ROOT"/daily_bundles_inbox
```

agentCLI 也可以直接指定平台和关键词：
...
python3 -m aetherflux_agentcli.cli run \
  ...
  --queries "阳朔 旅游" "阳朔 竹筏" \
  --main-inbox "$AETHERFLUX_DATA_ROOT"/daily_bundles_inbox
```
...
本机已有 `ffmpeg` 时可提取音频。ASR 后端按本地依赖自动选择：Apple Silicon 优先 `mlx-whisper` 或 `mlx_whisper` 命令行，其他环境优先 `faster-whisper`，缺依赖时会明确标记失败原因，不写假成功数据。

ASR 成功不等于情报有价值。agentCLI 会在 `asr_results.jsonl` 写入 `information_value`：

- `useful`：有排队、避坑、路线、预约、涨价、停运等可行动信息。
- `low_value`：纯配乐、重复歌词、幻觉式转写或没有明确情报信号。
- `review_needed`：有地点实体但价值不确定。
- `needs_ocr`：图文或滚动图片，先下载图片，后续进入 OCR/视觉识别。

外部抖音解析器默认禁用。已学习的 `douyin.txt` 插件源码会把目标抖音 URL 发给 `tiktokio.com` 并读取 `.tk-down-link`，只能在 GuGU 明确允许后作为备用下载方案。
...
旧 `/api/*` 只作为旧后台兼容参考，不再是 V0.2.7 的主接口。
...
换句话说：**V0.2.7 已经完成一轮代码审查修补；真实平台采集的稳定性仍要继续打磨。**
