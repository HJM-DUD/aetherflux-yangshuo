...
两个模式都要能产出同一种每日资料包，供后续“超级智脑”读取。资料包会在子项目本地保留一份，也可以复制到主项目统一入口 `data/daily_bundles_inbox/{mode}/{date}/{run_id}/`。
...
```bash
cd aetherflux_shellCLI
python3 -m aetherflux_shellcli.cli run --main-inbox ../data/daily_bundles_inbox
