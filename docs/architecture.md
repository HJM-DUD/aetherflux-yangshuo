...
阳朔旅游情报中心是“以太通量 / AetherFlux_yitaitongliang”主项目内的子项目。V0.2.5 在 V0.2.4 Web 后台之上，开始把核心采集系统拆成 `aetherflux_shellCLI` 和 `aetherflux_agentCLI` 两个可独立运行的子项目。前端为 React/Vite + Tailwind，后端为 FastAPI，主接口统一为 `/api/v1/*`。
...
## V0.2.6 paths.py（路径解析）

`aetherflux/paths.py` 是 V0.2.6 新增的统一路径解析模块：
...
7. 每天生成目录 + JSONL 形式的 `daily_bundle_YYYY-MM-DD/RUN_ID`，供第二部分“超级智脑”读取。
...
- V0.2.6：所有运行时数据迁移到 `AETHERFLUX_DATA_ROOT`（默认 `/Users/gugu/Documents/Agent/AetherFlux_Data`）。路径由 `aetherflux/paths.py` 统一解析。
